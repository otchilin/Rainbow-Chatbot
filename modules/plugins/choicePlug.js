  ...
  getNextStep(work, step, logger) {
    ...
    if (Array.isArray(step.next) && step.next.length > 1) {
      ...
      if (work.history) {
        ...
        // We search in history starting by the end
        // because the scenario can re-play the same step multiple times
        let historyItem = null;
        for (let i = work.history.length - 1; i >= 0; i--) {
          if (work.history[i].step === work.stepId) {

            historyItem = work.history[i];
            break;
          }
        }
