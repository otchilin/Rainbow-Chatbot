...
class Work {
  ...
  historize(msg) {
    ...
    // We search for step stating by the end of array, and take the 1st one

    for (let i = this._history.length - 1; i >= 0; i--) {
      if (this._history[i].step === that._stepId) {

        // Step found, we update it
        if (msg.value !== '') {
          this._history[i].content = msg.value;
        }
        if (msg.altContent) {
          this._history[i].altContent = msg.altContent;
        }
